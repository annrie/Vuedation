...
import { OrbitControlsExpansion } from "@/composables/OrbitControlsExpansion";
import Proton from "three.proton.js";
import { getGeometry } from "@/store/app.utils";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    renderer.setSize(
      state.stageWidth * state.deviceRatio,
      state.stageHight * state.deviceRatio
    );
    ...
    //背景
    const bgImages = [
      "/static/texture/hero-bg-top.png",
      "/static/texture/hero-bg-about.png",
      "/static/texture/hero-bg-author.png",
      "/static/texture/hero-bg-note.png",
      "/static/texture/hero-bg-qa.png",
      "/static/texture/hero-bg-form.png",
    ];

    const shaderTextureArr: THREE.Texture[] = [];
    const promises: Promise<THREE.Texture>[] = [];
    let bgGeometry: THREE.BufferGeometry,
      bgMaterial: ShaderMaterial,
      bgMaterial2: ShaderMaterial;
     ...
     */
    const createSprite = () => {
      // const map=new THREE.TextureLoader().load("/static/texture/triangle.png");
      const map = new THREE.TextureLoader().load("/static/texture/butterfly-1.png");
    ...
    const createEmitter = () => {
      ...
      emitter.addInitialize(
        new Proton.Velocity(0.2, new Proton.Vector3D(0, 0.1, 0.1), 0.2)
      );
    ...
    const onMouseMoveHandler = (e: MouseEvent) => {
      ...
      if (controls != null)
        controls.handleMouseMoveRotate(
          (e.clientX - state.stageWidth * 0.5) * state.evtAdjust,
          (e.clientY - state.stageHight * 0.5) * state.evtAdjust
        );
    };
    ...
    const bgChange = (nextPage: number) => {
      ...
      if (!state.isAnimation) {
        ...
        const afterNextTexture =
          shaderTextureArr[(nextPage + 1) % shaderTextureArr.length];
    ...
    const bgInit = () => {
      ...
      state.bgimageAspect =
        shaderTextureArr[0].image.naturalHeight / shaderTextureArr[0].image.naturalWidth;
    ...
    onMounted(() => {
      ...
      console.log(
        ">>AppHero >onMounted | currentPageIndex.value, props.nowPage : " +
    ...
    onUpdated(() => {
      console.log(
        ">>AppHero >onUpdated | currentPageIndex.value, props.nowPage : " +
          currentPageIndex.value +
          ", " +
          props.nowPage
      );
